...
function runSingle(propertiesToRun, reportsToRun) {
 ...
 if(!propertiesToRun || propertiesToRun.length === 0) {
  ...
  propertiesToRun = properties.filter((property) => propertiesToRun.includes(property.slug))
 }

 if(!reportsToRun || reportsToRun.length === 0) {
  throw new Error("Must specify at least one report"); 
 } else {
  reportsToRun = reports.filter((report) => reportsToRun.includes(report.name))
 }
  ...
  propertiesToRun.forEach((property) => {
      reportsToRun.forEach((report) => {
       ...
       try {
        uploadFileToCloudStorage(property.slug, report.name, data, storageToken); 
       } catch(err) {
        Logger.log("Failed to upload " + report.name + " report for " + property.name); 
       }
      });
    })
}

function runRunSingle() {
  runSingle(["dnrec"], ["top-pages-today"])
}
